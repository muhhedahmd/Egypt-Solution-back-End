...
function fixImageDimensions() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            try {
                // Get real dimensions
                const dimensions = yield getImageDimensions(image.url);
                if (dimensions) {
                    // Update database
...
function validateAndReport() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        // Check images
        const imageStats = yield prisma.image.groupBy({
            by: ['imageType'],
            _count: true,
            _avg: {
                width: true,
        ...
        const brokenImages = yield prisma.image.count({
            where: {
                OR: [
                    { width: 32 },
                    { height: 21 },
        ...
        // Check slideshows
        const slideshowStats = yield prisma.slideShow.findMany({
            select: {
                id: true,
                title: true,
                type: true,
        ...
        for (const slideshow of slideshowStats) {
            const total = slideshow._count.services +
...
// ============================================================================
function estimateDimensionsFromUrl() {
    return __awaiter(this, void 0, void 0, function* () {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        switch (mode) {
            case 'images':
                yield fixImageDimensions();
                break;
            case 'slideshows':
                yield cleanEmptySlideshows();
                break;
            case 'estimate':
                yield estimateDimensionsFromUrl();
                break;
            case 'report':
                yield validateAndReport();
                break;
            case 'all':
            default:
