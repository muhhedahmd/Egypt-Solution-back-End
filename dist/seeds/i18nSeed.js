...
function searchPexelsImage(query_1) {
    return __awaiter(this, arguments, void 0, function* (query, perPage = 1) {
        var _a;
        try {
            ...
            return (_a = response.data.photos) === null || _a === void 0 ? void 0 : _a[0];
        }
        catch (error) {
...
function downloadImage(url) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const response = yield axios_1.default.get(url, { responseType: 'arraybuffer', timeout: 30000 });
...
function getRealImage(query_1) {
    return __awaiter(this, arguments, void 0, function* (query, fallbackSize = '1200x800') {
        var _a;
        try {
            const photo = yield searchPexelsImage(query);
            if ((_a = photo === null || photo === void 0 ? void 0 : photo.src) === null || _a === void 0 ? void 0 : _a.large2x) {
                console.log(`  📸 Found Pexels image: "${photo.alt}" by ${photo.photographer}`);
                const imageBuffer = yield downloadImage(photo.src.large2x);
                if (imageBuffer)
                    return imageBuffer;
            }
            ...
            return yield downloadImage(placeholderUrl);
        }
        catch (error) {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield prisma.serviceTranslation.deleteMany();
        yield prisma.service.deleteMany();
        yield prisma.projectTranslation.deleteMany();
        yield prisma.projectTechnology.deleteMany();
        yield prisma.project.deleteMany();
        yield prisma.clientTranslation.deleteMany();
        yield prisma.client.deleteMany();
        yield prisma.testimonialTranslation.deleteMany();
        yield prisma.testimonial.deleteMany();
        yield prisma.teamMemberTranslation.deleteMany();
        ...
        for (let i = 0; i < 20; i++) {
            ...
            try {
                ...
                const imageBuffer = yield getRealImage(serviceData.searchQuery, '1200x800');
                if (!imageBuffer)
                    continue;
                yield delay(500);
                const uploadedImage = yield (0, helpers_1.UploadImage)(imageBuffer, `${serviceData.en.name} ${i + 1}`);
                if (!uploadedImage)
                    continue;
                ...
                const service = yield prisma.service.create({
                    data: {
                        ...
                        slug: (0, slugify_1.default)(`${serviceData.en.name}-${(0, crypto_1.randomUUID)().substring(0, 6)}`, { lower: true }),
                        ...
                        serviceTranslation: {
                            create: [
                                {
                                    lang: client_1.EnumLang.EN,
                                    name: serviceData.en.name,
                                    description: serviceData.en.description,
                                    richDescription: serviceData.en.richDescription,
                                },
                                {
                                    lang: client_1.EnumLang.AR,
        ...
        for (let i = 0; i < 20; i++) {
            try {
                const enTitle = `${faker_1.faker.company.catchPhrase()} Platform`;
                const arTitle = `منصة ${faker_1.faker.company.name()}`;
                const enDesc = faker_1.faker.lorem.sentence(20);
                const arDesc = faker_1.faker.lorem.sentence(20);
                const enRich = faker_1.faker.lorem.paragraphs(4);
                const arRich = faker_1.faker.lorem.paragraphs(4);
                console.log(`\n[${i + 1}/20] Creating: ${enTitle}...`);
                const imageBuffer = yield getRealImage(projectQueries[i % projectQueries.length], '1400x900');
                if (!imageBuffer)
                    continue;
                yield delay(500);
                const uploadedImage = yield (0, helpers_1.UploadImage)(imageBuffer, enTitle);
                if (!uploadedImage)
                    continue;
                const dbImage = yield (0, helpers_1.AssignImageToDBImage)({
                    imageType: 'PROJECT',
                ...
                const project = yield prisma.project.create({
                    data: {
                        ...
                        slug: (0, slugify_1.default)(`${enTitle}-${(0, crypto_1.randomUUID)().substring(0, 6)}`, { lower: true }),
                        clientName: faker_1.faker.person.fullName(),
                        clientCompany: faker_1.faker.company.name(),
                        projectUrl: Math.random() > 0.3 ? faker_1.faker.internet.url() : undefined,
                        status: faker_1.faker.helpers.enumValue(client_1.ProjectStatus),
                        startDate: faker_1.faker.date.past({ years: 2 }),
                        endDate: Math.random() > 0.3 ? faker_1.faker.date.recent({ days: 180 }) : undefined,
                        imageId: dbImage.id,
                        isFeatured: i < 8,
                        order: i,
                        ProjectTranslation: {
        ...
        for (let i = 0; i < 20; i++) {
            try {
                ...
                const enDesc = faker_1.faker.company.catchPhrase();
                const arDesc = faker_1.faker.company.catchPhrase();
                const enRich = faker_1.faker.lorem.paragraphs(3);
                const arRich = faker_1.faker.lorem.paragraphs(3);
                const industry = industries[i % industries.length];
                console.log(`\n[${i + 1}/20] Creating: ${companyName}...`);
                const imageBuffer = yield getRealImage('modern office building', '1200x800');
                const logoBuffer = yield getRealImage('company logo minimal', '400x400');
                if (!imageBuffer || !logoBuffer)
                    continue;
                yield delay(500);
                const uploadedImage = yield (0, helpers_1.UploadImage)(imageBuffer, `${companyName} office`);
                const uploadedLogo = yield (0, helpers_1.UploadImage)(logoBuffer, `${companyName} logo`);
                ...
                const dbLogo = yield (0, helpers_1.AssignImageToDBImage)({
                    imageType: 'CLIENT',
                ...
                const client = yield prisma.client.create({
                    data: {
                        ...
                        richDescription: enRich,
                        industry: industry,
                        slug: (0, slugify_1.default)(`${companyName}-${(0, crypto_1.randomUUID)().substring(0, 6)}`, { lower: true }),
        ...
        for (let i = 0; i < 20; i++) {
            try {
                ...
                const bio = faker_1.faker.lorem.paragraphs(2);
                console.log(`\n[${i + 1}/20] Creating: ${fullName}...`);
                const imageBuffer = yield getRealImage('professional headshot portrait', '600x600');
                if (!imageBuffer)
                    continue;
                yield delay(1000);
        ...
        for (let i = 0; i < 20; i++) {
            try {
                ...
                const imageBuffer = yield getRealImage('professional portrait smiling', '400x400');
                if (!imageBuffer)
                    continue;
                yield delay(1000);
                const uploadedImage = yield (0, helpers_1.UploadImage)(imageBuffer, fullName);
                ...
                const companyName = faker_1.faker.company.name();
                const position = faker_1.faker.person.jobTitle();
                const content = faker_1.faker.lorem.paragraph({ min: 3, max: 6 });
            ...
            try {
                ...
                const slideshow = yield prisma.slideShow.create({
                    data: {
                        ...
                        slug: (0, slugify_1.default)(`${config.en.title}-${(0, crypto_1.randomUUID)().substring(0, 6)}`, { lower: true }),
                        type: config.type,
                ...
                switch (config.attachType) {
                    case 'services':
                        const servicesToAttach = faker_1.faker.helpers.arrayElements(createdServices, Math.min(config.attachCount, createdServices.length));
                        ...
                        break;
                    case 'projects':
                        const projectsToAttach = faker_1.faker.helpers.arrayElements(createdProjects, Math.min(config.attachCount, createdProjects.length));
                        ...
                        break;
                    case 'clients':
                        const clientsToAttach = faker_1.faker.helpers.arrayElements(createdClients, Math.min(config.attachCount, createdClients.length));
                        ...
                        break;
                    case 'testimonials':
                        const testimonialsToAttach = faker_1.faker.helpers.arrayElements(createdTestimonials, Math.min(config.attachCount, createdTestimonials.length));
                        ...
                        break;
                    case 'team':
                        const teamToAttach = faker_1.faker.helpers.arrayElements(createdTeamMembers, Math.min(config.attachCount, createdTeamMembers.length));
        ...
        // ============================================================================
        const finalCounts = {
            services: yield prisma.service.count(),
            projects: yield prisma.project.count(),
            clients: yield prisma.client.count(),
            teamMembers: yield prisma.teamMember.count(),
            testimonials: yield prisma.testimonial.count(),
            slideshows: yield prisma.slideShow.count(),
            serviceTranslations: yield prisma.serviceTranslation.count(),
            projectTranslations: yield prisma.projectTranslation.count(),
            clientTranslations: yield prisma.clientTranslation.count(),
            teamTranslations: yield prisma.teamMemberTranslation.count(),
            testimonialTranslations: yield prisma.testimonialTranslation.count(),
            slideshowTranslations: yield prisma.slideShowTranslation.count(),
        };
        console.log('\n\n✅ ==========================================');
