...
function seedAnalytics() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let dayAgo = DAYS_TO_SEED - 1; dayAgo >= 0; dayAgo--) {
            const date = getDateDaysAgo(dayAgo);
            yield aggregateDay(date);
        }
        console.log('✅ Analytics aggregation complete!');
        // Print summary
        const totalSessions = yield prisma.sessionAnalytics.count();
        const totalPageViews = yield prisma.pageView.count();
        const totalAnalyticsDays = yield prisma.analytics.count();
        console.log('\n📈 Summary:');
...
function aggregateDay(date) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const sessions = yield prisma.sessionAnalytics.findMany({
            where: { startedAt: { gte: startOfDay, lte: endOfDay } },
        });
        const pageViews = yield prisma.pageView.findMany({
